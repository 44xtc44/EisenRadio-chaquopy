 ...
 *
 * Credit to GitHub package com.jventura.pybridge;
 *   alternative dst for copyAssets(src, dst) by 44xtc44
 */
...
public class AssetExtractor extends AppCompatActivity {

    private final static String LOG_TAG = "[AssetExtractor]";
    ...
    private void copyAssetFile(String src, String dst) {
        ...
        Log.i(LOG_TAG, String.format("Copying %s -> %s", src, dst));
     ...
     *
     * @param pathSource: the source path
     * @param pathDest: the destination path
     */
    public void copyAssets(String pathSource, String pathDest) {
        boolean hasDestination = pathDest.length() > 0;

        for (String asset : listAssets(pathSource)) {
            if (hasDestination) {
                copyAssetFile(asset, pathDest + asset);
            } else {
    ...
    private void recursiveDelete(File file) {
        ...
        Log.i(LOG_TAG, "Removing " + file.getAbsolutePath());
        file.delete();
    }
